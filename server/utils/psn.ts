...
import { getDoubanWatchedStore } from './douban-auth'
import seedProfile from '../data/psn-shallwetalk2022.json'
...
export const fetchPsnProfile = async (psnId: string): Promise<PsnProfile> => {
...
const memoryProfiles = new Map<string, PsnProfile>()

export const getPsnProfile = async (event: H3Event, psnId: string): Promise<PsnProfile> => {
  const cached = memoryProfiles.get(psnId)
  if (cached) return cached

  const store = getDoubanWatchedStore(event)
  if (store) {
    try {
      const stored = await store.get<PsnProfile>(`${PSN_STORAGE_KEY_PREFIX}${psnId}`, 'json')
      if (stored && Array.isArray(stored.games)) {
        memoryProfiles.set(psnId, stored)
        return stored
      }
    } catch {
      // ignore and fallback
  ...
  try {
    const nitroStored = await useStorage('douban').getItem<PsnProfile>(`${PSN_STORAGE_KEY_PREFIX}${psnId}`)
    if (nitroStored && Array.isArray(nitroStored.games)) {
      memoryProfiles.set(psnId, nitroStored)
      return nitroStored
    }
  } catch {
    // ignore
  ...
  if (psnId.toLowerCase() === seedProfile.id.toLowerCase()) {
    const seed = seedProfile as PsnProfile
    memoryProfiles.set(psnId, seed)
    return seed
  }

  const fresh = await fetchPsnProfile(psnId)
  await savePsnProfile(event, psnId, fresh)
  return fresh
}

export const savePsnProfile = async (event: H3Event, psnId: string, profile: PsnProfile): Promise<PsnProfile> => {
  const toSave: PsnProfile = {
    ...profile,
    updatedAt: new Date().toISOString()
  }

  memoryProfiles.set(psnId, toSave)

  const store = getDoubanWatchedStore(event)
  if (store) {
    try {
      await store.put(`${PSN_STORAGE_KEY_PREFIX}${psnId}`, JSON.stringify(toSave))
    } catch (error) {
      console.error('Failed to write PSN profile to Cloudflare KV', error)
    }
  }

  try {
    await useStorage('douban').setItem(`${PSN_STORAGE_KEY_PREFIX}${psnId}`, toSave)
  } catch (error) {
    console.error('Failed to write PSN profile to local storage', error)
  }
